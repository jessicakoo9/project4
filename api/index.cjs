...
app.get("/api/by_name/:qname", (req, res) => {
  const q = (req.params.qname || "").toLowerCase();
  const filtered = employees.filter((e) => e.employee_name.toLowerCase().includes(q));
  res.json({ employees: filtered });
});

app.get("/api/by_age/:min/:max", (req, res) => {
  const min = parseInt(req.params.min, 10);
  const max = parseInt(req.params.max, 10);

  const filtered = employees.filter((e) => e.employee_age >= min && e.employee_age <= max);
  res.json({ employees: filtered });
});
...
app.get("/api/search", (req, res) => {
  ...
  const filtered = employees.filter((e) => {
    const matchesName = name ? e.employee_name.toLowerCase().includes(name) : true;
    const matchesMin = minAge !== null ? e.employee_age >= minAge : true;
    const matchesMax = maxAge !== null ? e.employee_age <= maxAge : true;
